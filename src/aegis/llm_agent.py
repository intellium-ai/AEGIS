...
from aegis.prompt import CHOOSE_ACTION

logger = logging.getLogger(__name__)


class LLMAction(BaseModel):
    action: int = Field(..., description="The action for the LLM to take")


class LLMPolicy(BasePolicy):

    def __init__(self, client: Client, *args, squash_output: bool = False, **kwargs):
        super().__init__(*args, squash_output=squash_output, **kwargs)
        self.client = client
        logger.info("LLM agent initialised")

    def _predict(self, observation: Tensor, deterministic: bool = False) -> Tensor:
        # assigning to a variable here to help with type hints
        prompt = CHOOSE_ACTION.format(observation=observation)
        logger.info(prompt)
        response = self.client.generate(
            prompt=prompt,
            do_sample=deterministic,
            grammar=Grammar(type=GrammarType.Json, value=LLMAction.model_json_schema()),
        )

        action_response = LLMAction(**json.loads(response.generated_text))
        logging.info(action_response)
        return Tensor([action_response.action])
